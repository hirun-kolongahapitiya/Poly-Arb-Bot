...
const getOrderBookErrorMessage = (error: unknown): string | undefined => {
    if (error && typeof error === 'object') {
        const maybeResponse = (error as { response?: { data?: unknown } }).response;
        const extracted = extractOrderError(maybeResponse?.data);
        if (extracted) {
            return extracted;
        }
    }
...
const isOrderBookMissingError = (error: unknown, message?: string): boolean => {
    ...
    const lower = message.toLowerCase();
    return lower.includes('no orderbook exists');
};
    ...
    if (condition === 'merge') {
        ...
        while (remaining > 0 && retry < RETRY_LIMIT) {
            ...
            try {
                ...
                const errorMessage = getOrderBookErrorMessage(error);
                if (isOrderBookMissingError(error, errorMessage)) {
                    Logger.warning(
                        `No orderbook exists for token ${trade.asset} - skipping trade`
                    );
                    await UserActivity.updateOne({ _id: trade._id }, { bot: true });
                    return;
                }
                retry += 1;
                Logger.warning(
                    `Orderbook fetch failed (attempt ${retry}/${RETRY_LIMIT})${errorMessage ? ` - ${errorMessage}` : ''}`
                );
