...
const TOO_OLD_MINUTES = ENV.TOO_OLD_TIMESTAMP_MINUTES;
...
const fetchTradeData = async () => {
    for (const { address, UserActivity, UserPosition } of userModels) {
        try {
            ...
            const tooOldSeconds =
                typeof TOO_OLD_MINUTES === 'number'
                    ? TOO_OLD_MINUTES * 60
                    : TOO_OLD_HOURS * 3600;
            const cutoffTimestamp = Math.floor(Date.now() / 1000) - tooOldSeconds;

            // Process each activity
            for (const activity of activities) {
                // Skip if too old
                const activityTimestamp =
                    activity.timestamp > 1_000_000_000_000
...
const tradeMonitor = async () => {
    ...
    try {
        ...
        Logger.error(`Trade monitor error: ${error}`);
    } finally {
