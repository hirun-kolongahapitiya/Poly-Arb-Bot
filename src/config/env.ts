...
const validateNumericConfig = (): void => {
    ...
    const tooOldMinutesRaw = process.env.TOO_OLD_TIMESTAMP_MINUTES;
    if (typeof tooOldMinutesRaw === 'string' && tooOldMinutesRaw.trim() !== '') {
        const tooOldMinutes = parseInt(tooOldMinutesRaw, 10);
        if (isNaN(tooOldMinutes) || tooOldMinutes < 1) {
            throw new Error(
                `Invalid TOO_OLD_TIMESTAMP_MINUTES: ${process.env.TOO_OLD_TIMESTAMP_MINUTES}. Must be a positive integer (minutes).`
            );
        }
    }
...
const validateUrls = (): void => {
    ...
    if (process.env.STORAGE_DIR && process.env.STORAGE_DIR.trim() === '') {
        console.error('\n❌ Invalid STORAGE_DIR\n');
        console.error('STORAGE_DIR cannot be empty if provided.\n');
        throw new Error('Invalid STORAGE_DIR: must be a non-empty path.');
    }
};
...
export const ENV = {
    ...
    TOO_OLD_TIMESTAMP_MINUTES:
        typeof process.env.TOO_OLD_TIMESTAMP_MINUTES === 'string' &&
        process.env.TOO_OLD_TIMESTAMP_MINUTES.trim() !== ''
            ? parseInt(process.env.TOO_OLD_TIMESTAMP_MINUTES, 10)
            : undefined,
