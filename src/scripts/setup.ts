...
async function setupRPC(): Promise<string> {
    printSection('STEP 3: POLYGON RPC ENDPOINT');
...
async function setupStrategy(): Promise<{
    ...
    printSection('STEP 4: TRADING STRATEGY (OPTIONAL)');
...
async function setupRiskLimits(): Promise<{ maxOrder: string; minOrder: string }> {
    printSection('STEP 5: RISK LIMITS (OPTIONAL)');
...
async function main() {
    ...
    try {
        ...
        console.log(`  ${colors.red}☐${colors.reset} Verify storage folder is writable\n`);
